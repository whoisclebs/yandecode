...
describe('applyBoosts', () => {
  it('boosts when a query token matches the last segment of the symbol, case-insensitively', () => {
    const out = applyBoosts(
      [{ id: 1, score: 0.5, symbol: 'AuthService.Login', path: 'src/auth.ts' }],
      'login flow',
    );
  ...
  it('boosts test-path candidates when the query mentions test or spec', () => {
    const out = applyBoosts(
      [{ id: 1, score: 0.5, symbol: null, path: 'test/auth.integration.test.ts' }],
  ...
  it('applies both boosts when both conditions hold', () => {
    const out = applyBoosts(
      [{ id: 1, score: 0.5, symbol: 'AuthService.Login', path: 'test/auth.test.ts' }],
      'login test',
    );
  ...
  it('boosts a path where the test/spec directory is the very first path segment', () => {
    const out = applyBoosts(
      [{ id: 1, score: 0.5, symbol: null, path: 'test/handler.ts' }],
      'run the test suite',
    );
    expect(out[0]?.score).toBeCloseTo(0.52, 10);
  });
  ...
  it('boosts a leading spec/ directory the same way', () => {
    const out = applyBoosts(
      [{ id: 1, score: 0.5, symbol: null, path: 'spec/foo.ts' }],
      'spec coverage',
    );
    expect(out[0]?.score).toBeCloseTo(0.52, 10);
  });
