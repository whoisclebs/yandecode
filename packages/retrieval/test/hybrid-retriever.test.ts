...
const SINGLETON_TOPK: HybridRetrieverConstants = { denseTopK: 1, lexicalTopK: 1, rrfK: 60, mmrLambda: 0.7, maxResults: 12, tokenBudget: 6000 };
...
async function addChunk(path: string, content: string, embeddedText: string): Promise<void> {
  ...
  const { inserted } = await documents.replaceDocument(
    { path, language: 'text', sizeBytes: content.length, contentHash: path, gitCommit: null, indexGeneration: 1 },
    [{ kind: 'text', symbol: null, identifiers: content.toLowerCase(), startLine: 1, endLine: 1, content, contentHash: path, tokenCount: 20, embedding: embedding ?? null }],
  );
...
beforeEach(async () => {
  ...
  index = new USearchVectorIndex({ dimensions: provider.dimensions, file: join(root, 'repository-00001.usearch') });

  // Dense-only: no literal overlap with the query in its stored content, but embedded from text close to the query.
  await addChunk('src/dense-only.ts', 'rotates the signing material on a schedule', 'token rotation policy');
  // Lexical-only: literal query tokens in its stored content, but embedded from unrelated text.
  await addChunk('src/lexical-only.ts', 'this file documents token rotation for session credentials', 'weather forecast for tomorrow');
...
describe('HybridRetriever', () => {
  it('fuses a dense-only match and a lexical-only match that neither single-best channel alone would return together', async () => {
    const retriever = new HybridRetriever({ documents, provider, index, constants: SINGLETON_TOPK });
