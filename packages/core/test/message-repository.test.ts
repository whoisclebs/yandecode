...
describe('MessageRepository', () => {
  it('sends a message, round-trips the payload as parsed JSON, and lists it as unread', async () => {
    ...
    const msg = await messages.send({ swarmId, from: 'task-scout', to: 'task-implementer', type: 'finding', payload });
  ...
  it('markRead removes a message from the unread list', async () => {
    const { state, messages, swarmId } = await setup();
    const msg = await messages.send({ swarmId, from: 'a', to: 'b', type: 'question', payload: { q: 'why?' } });
  ...
  it('rejects a payload larger than 16 KB', async () => {
    ...
    await expect(messages.send({ swarmId, from: 'a', to: 'b', type: 'warning', payload: huge })).rejects.toThrow(/MESSAGE_PAYLOAD_TOO_LARGE/);
    state.close();
  });
  ...
  it('rejects an invalid message type at the database layer', async () => {
    ...
    await expect(messages.send({ swarmId, from: 'a', to: 'b', type: 'not-a-type', payload: {} })).rejects.toThrow();
    state.close();
  });
});
