...
describe('StateService', () => {
  ...
  it('serializes writes in FIFO order inside transactions', async () => {
    ...
    const writes = [1, 2, 3].map((n) =>
      state.write((db) => {
        order.push(n);
        db.prepare(
          "INSERT INTO index_dirty (path, reason, marked_at) VALUES (?, 'test', '2026-01-01T00:00:00.000Z')",
        ).run(`f${n}`);
        return n;
      }),
    );
    ...
    expect(state.read((db) => db.prepare('SELECT COUNT(*) AS c FROM index_dirty').get())).toEqual({
      c: 3,
    });
    state.close();
  });
  ...
  it('rolls back a failing write and keeps serving later writes', async () => {
    ...
    await expect(
      state.write((db) => {
        db.prepare(
          "INSERT INTO index_dirty (path, reason, marked_at) VALUES ('a', 'x', 'now')",
        ).run();
        throw new Error('boom');
      }),
    ).rejects.toThrow('boom');
    await state.write((db) =>
      db
        .prepare("INSERT INTO index_dirty (path, reason, marked_at) VALUES ('b', 'x', 'now')")
        .run(),
    );
