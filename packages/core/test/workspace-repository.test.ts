...
describe('WorkspaceRepository', () => {
  it('creates a worktree workspace and reads it back', async () => {
    ...
    const ws = await workspaces.create({ swarmId, kind: 'worktree', name: 'task-implement-auth', path: '/repo/.worktrees/task-implement-auth', branch: 'task/implement-auth' });
  ...
  it('listBySwarm returns every workspace for that swarm; markRemoved sets removedAt', async () => {
    ...
    const b = await workspaces.create({ swarmId, kind: 'worktree', name: 'wt', path: '/repo/.worktrees/wt' });
    expect(workspaces.listBySwarm(swarmId).map((w) => w.id).sort()).toEqual([a.id, b.id].sort());
