...
async function setup() {
  ...
  const [task] = await tasks.createMany([
    { swarmId: swarm.id, title: 't', description: 'd', role: 'implementer' },
  ]);
...
describe('LeaseRepository', () => {
  it('creates a lease with a 15-minute default TTL and reads it back among active leases', async () => {
    ...
    expect(new Date(lease.expiresAt).getTime() - before).toBeGreaterThanOrEqual(
      15 * 60 * 1000 - 1000,
    );
  ...
  it('excludes a released lease and an expired lease from listActive', async () => {
    ...
    const expired = await leases.create({
      swarmId,
      taskId,
      pattern: 'b/**',
      holderAgent: 'x',
  ...
  it("releaseAllForTask releases only that task's leases, leaving a sibling task's leases active", async () => {
    ...
    const [otherTask] = await tasks.createMany([
      { swarmId, title: 't2', description: 'd2', role: 'implementer' },
    ]);
    const mine = await leases.create({ swarmId, taskId, pattern: 'a/**', holderAgent: 'x' });
    const theirs = await leases.create({
      swarmId,
      taskId: otherTask!.id,
      pattern: 'b/**',
      holderAgent: 'x',
  ...
  it("releaseAllForSwarm releases only that swarm's leases, leaving another swarm's leases active", async () => {
    ...
    const otherSwarm = await swarms.create({
      title: 't2',
      goal: 'g2',
      strategy: 'adaptive',
      maxAgents: 4,
    ...
    const [otherTask] = await tasks.createMany([
      { swarmId: otherSwarm.id, title: 't', description: 'd', role: 'implementer' },
    ]);
    const mine = await leases.create({ swarmId, taskId, pattern: 'a/**', holderAgent: 'x' });
    const theirs = await leases.create({
  ...
  it('reserveIfNoConflict grants and creates leases atomically when there is no conflict', async () => {
    const { state, leases, swarmId, taskId } = await setup();
    const result = await leases.reserveIfNoConflict(
      { swarmId, taskId, patterns: ['a/**', 'b/**'], holderAgent: 'x' },
      () => [],
    );
  ...
  it('reserveIfNoConflict denies and creates nothing when the conflict callback reports a conflict', async () => {
    ...
    const result = await leases.reserveIfNoConflict(
      { swarmId, taskId, patterns: ['a/**'], holderAgent: 'y' },
      (_pattern, active) => active,
    );
