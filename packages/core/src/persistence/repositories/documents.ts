...
export class DocumentRepository {
  ...
  listDocuments(): { id: string; path: string; contentHash: string }[] {
    const rows = this.state.read((db) => db.prepare('SELECT id, path, content_hash FROM documents ORDER BY path').all()) as {
      id: string;
      path: string;
      content_hash: string;
    }[];
  ...
  replaceDocument(doc: DocumentInput, chunks: ChunkInput[]): Promise<{ documentId: string; inserted: ChunkRecord[]; removedVectorIds: number[] }> {
    return this.state.write((db) => {
      const removedVectorIds = (db.prepare('SELECT c.vector_id AS v FROM chunks c JOIN documents d ON d.id = c.document_id WHERE d.path = ?').all(doc.path) as { v: number }[]).map((r) => r.v);
      ...
      ).run(documentId, doc.path, doc.language, doc.sizeBytes, doc.contentHash, doc.gitCommit, now, doc.indexGeneration);
      const seq = db.prepare("UPDATE vector_id_seq SET next_id = next_id + ? WHERE name = 'repository' RETURNING next_id").get(chunks.length) as { next_id: number };
      ...
      for (const c of chunks) {
        const id = newId();
        insert.run(id, documentId, vectorId, c.kind, c.symbol, c.identifiers, c.startLine, c.endLine, c.content, c.contentHash, c.tokenCount, c.embedding ? vectorToBlob(c.embedding) : null, now, now);
        inserted.push({ id, documentId, path: doc.path, vectorId, kind: c.kind, symbol: c.symbol, identifiers: c.identifiers, startLine: c.startLine, endLine: c.endLine, content: c.content, contentHash: c.contentHash, tokenCount: c.tokenCount });
        vectorId += 1;
      }
  ...
  deleteDocument(path: string): Promise<number[]> {
    return this.state.write((db) => {
      const ids = (db.prepare('SELECT c.vector_id AS v FROM chunks c JOIN documents d ON d.id = c.document_id WHERE d.path = ?').all(path) as { v: number }[]).map((r) => r.v);
  ...
  chunksByVectorIds(ids: number[]): ChunkRecord[] {
    ...
    const rows = this.state.read((db) => db.prepare(`${CHUNK_SELECT} WHERE c.vector_id IN (${placeholders})`).all(...ids)) as ChunkRow[];
  ...
  embeddingsByVectorIds(ids: number[]): Map<number, Float32Array> {
    ...
    const placeholders = ids.map(() => '?').join(',');
    const rows = this.state.read((db) => db.prepare(`SELECT vector_id, embedding FROM chunks WHERE vector_id IN (${placeholders}) AND embedding IS NOT NULL`).all(...ids)) as { vector_id: number; embedding: Buffer }[];
  ...
  *iterateEmbeddings(): IterableIterator<{ vectorId: number; embedding: Float32Array }> {
    const stmt = this.state.db.prepare('SELECT vector_id, embedding FROM chunks WHERE embedding IS NOT NULL ORDER BY vector_id');
  ...
  allVectorIds(): number[] {
    return (this.state.read((db) => db.prepare('SELECT vector_id AS v FROM chunks ORDER BY vector_id').all()) as { v: number }[]).map((r) => r.v);
  }
}
