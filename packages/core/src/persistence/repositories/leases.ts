...
export class LeaseRepository {
  ...
  create(input: {
    ...
    return this.state.write((db) => {
      ...
      db.prepare(
        'INSERT INTO leases (id, swarm_id, task_id, pattern, holder_agent, acquired_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
      ).run(
        id,
        input.swarmId,
        input.taskId,
  ...
  listActive(swarmId: string, now: string = nowIso()): LeaseRecord[] {
    const rows = this.state.read((db) =>
      db
        .prepare(
          'SELECT * FROM leases WHERE swarm_id = ? AND released_at IS NULL AND expires_at > ? ORDER BY acquired_at',
        )
        .all(swarmId, now),
    ) as Row[];
  ...
  releaseAllForTask(taskId: string): Promise<void> {
    return this.state.write((db) => {
      db.prepare('UPDATE leases SET released_at = ? WHERE task_id = ? AND released_at IS NULL').run(
        nowIso(),
        taskId,
      );
    });
  ...
  releaseAllForSwarm(swarmId: string): Promise<void> {
    return this.state.write((db) => {
      db.prepare(
        'UPDATE leases SET released_at = ? WHERE swarm_id = ? AND released_at IS NULL',
      ).run(nowIso(), swarmId);
    });
  }
  ...
  reserveIfNoConflict(
    ...
    return this.state.write((db) => {
      ...
      for (const pattern of input.patterns) {
        ...
        db.prepare(
          'INSERT INTO leases (id, swarm_id, task_id, pattern, holder_agent, acquired_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
        ).run(id, input.swarmId, input.taskId, pattern, input.holderAgent, acquiredAt, expiresAt);
        leases.push({
          id,
          swarmId: input.swarmId,
          taskId: input.taskId,
