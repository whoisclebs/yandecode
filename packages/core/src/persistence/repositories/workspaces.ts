...
export class WorkspaceRepository {
  ...
  create(input: {
    ...
    return this.state.write((db) => {
      ...
      db.prepare(
        'INSERT INTO workspaces (id, swarm_id, kind, name, path, branch, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
      ).run(id, input.swarmId, input.kind, input.name, input.path, branch, now);
      return {
        id,
  ...
  listBySwarm(swarmId: string): WorkspaceRecord[] {
    const rows = this.state.read((db) =>
      db.prepare('SELECT * FROM workspaces WHERE swarm_id = ? ORDER BY created_at').all(swarmId),
    ) as Row[];
