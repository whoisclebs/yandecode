...
class FakeEmbeddingProvider implements EmbeddingProvider {
  ...
  embedDocument(text: string): Promise<Float32Array> { return Promise.resolve(this.vectors.get(text) ?? new Float32Array([0, 0, 1])); }
...
function setup(vectors: Map<string, Float32Array> = new Map()) {
  const state = StateService.open(join(mkdtempSync(join(tmpdir(), 'yc-memory-retriever-')), 'state.db'));
  const memories = new MemoryRepository(state);
  const index = new FakeVectorIndex();
  const provider = new FakeEmbeddingProvider(vectors);
  return { state, memories, index, provider, retriever: new MemoryRetriever({ memories, provider, index }) };
}
...
describe('MemoryRetriever.search', () => {
  ...
  it('fuses dense and lexical hits and returns the best-matching memory first', async () => {
    ...
    const a = await memories.create({ namespace: 'patterns', content: 'retry backoff', summary: 'retry pattern', sourceSwarmId: null, sourceTaskId: null, confidence: 0.5, contentHash: 'a', embedding: vA });
    const b = await memories.create({ namespace: 'failures', content: 'deploy timeout', summary: 'deploy failure', sourceSwarmId: null, sourceTaskId: null, confidence: 0.5, contentHash: 'b', embedding: vB });
  ...
  it('filters out dense-matched memories outside the requested namespace', async () => {
    ...
    const { state, memories, index, retriever } = setup(vectors);
    const pattern = await memories.create({ namespace: 'patterns', content: 'a pattern about retries', summary: null, sourceSwarmId: null, sourceTaskId: null, confidence: 0.5, contentHash: 'p', embedding: shared });
    const failure = await memories.create({ namespace: 'failures', content: 'a failure about retries', summary: null, sourceSwarmId: null, sourceTaskId: null, confidence: 0.5, contentHash: 'f', embedding: shared });
  ...
  it('respects the limit option', async () => {
    ...
    for (const [content, vec] of vectors) {
      const rec = await memories.create({ namespace: 'patterns', content, summary: null, sourceSwarmId: null, sourceTaskId: null, confidence: 0.5, contentHash: content, embedding: vec });
...
describe('applyRecencyConfidenceBoost', () => {
  it('adds confidence * CONFIDENCE_BOOST_WEIGHT plus a full recency boost for a just-updated memory', () => {
    const now = Date.now();
    const [boosted] = applyRecencyConfidenceBoost([{ id: 1, score: 0.5, confidence: 1, updatedAt: new Date(now).toISOString() }], now);
    expect(boosted!.score).toBeCloseTo(0.5 + 1 * CONFIDENCE_BOOST_WEIGHT + 1 * RECENCY_BOOST_WEIGHT, 10);
  });
  ...
  it('decays the recency boost linearly to zero at RECENCY_DECAY_DAYS and floors at zero beyond it', () => {
    ...
    const [halfBoosted] = applyRecencyConfidenceBoost([{ id: 1, score: 0, confidence: 0, updatedAt: halfway }], now);
    const [oldBoosted] = applyRecencyConfidenceBoost([{ id: 2, score: 0, confidence: 0, updatedAt: old }], now);
