...
describe('evaluateWritePolicy', () => {
  it('allows valid content with evidence and in-range confidence', () => {
    const result = evaluateWritePolicy({
      content: 'Retry idle DB connections with exponential backoff.',
      evidence: 'task-123',
      confidence: 0.7,
    });
  ...
  it('rejects empty or whitespace-only content', () => {
    expect(
      evaluateWritePolicy({ content: '   ', evidence: 'task-123', confidence: 0.5 }).allowed,
    ).toBe(false);
  });
  ...
  it('rejects content over the size cap', () => {
    ...
    expect(result).toEqual({
      allowed: false,
      reason: `content exceeds ${MAX_MEMORY_CONTENT_CHARS} characters`,
    });
  });

  it('rejects content with no evidence', () => {
    expect(
      evaluateWritePolicy({ content: 'a useful fact', evidence: null, confidence: 0.5 }).allowed,
    ).toBe(false);
    expect(
      evaluateWritePolicy({ content: 'a useful fact', evidence: '   ', confidence: 0.5 }).allowed,
    ).toBe(false);
  });

  it('rejects confidence outside [0, 1]', () => {
    expect(
      evaluateWritePolicy({ content: 'a useful fact', evidence: 'task-1', confidence: 1.1 })
        .allowed,
    ).toBe(false);
    expect(
      evaluateWritePolicy({ content: 'a useful fact', evidence: 'task-1', confidence: -0.1 })
        .allowed,
    ).toBe(false);
  });
  ...
  it('rejects content containing an AWS access key id', () => {
    const result = evaluateWritePolicy({
      content: 'the key was AKIAABCDEFGHIJKLMNOP embedded in config',
      evidence: 'task-1',
      confidence: 0.5,
    });
  ...
  it('rejects content containing an inline api_key/token/password assignment', () => {
    expect(
      evaluateWritePolicy({
        content: 'api_key: "sk-ab12cd34ef56gh78ij90"',
        evidence: 'task-1',
        confidence: 0.5,
      }).allowed,
    ).toBe(false);
  ...
  it('rejects content containing a SECRET_KEY-style assignment', () => {
    expect(
      evaluateWritePolicy({
        content: 'SECRET_KEY = "abcdefghijklmnop123"',
        evidence: 'task-1',
        confidence: 0.5,
      }).allowed,
    ).toBe(false);
    expect(
      evaluateWritePolicy({
        content: 'secret_key: "wJalrXUtnFEMIK7MDENGbPxRfiCY"',
        evidence: 'task-1',
        confidence: 0.5,
      }).allowed,
    ).toBe(false);
  ...
  it('rejects content containing an AWS temporary session credential (ASIA prefix)', () => {
    const result = evaluateWritePolicy({
      content: 'the token was ASIAABCDEFGHIJKLMNOP in the logs',
      evidence: 'task-1',
      confidence: 0.5,
    });
    expect(result.allowed).toBe(false);
