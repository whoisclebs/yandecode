...
class FakeEmbeddingProvider implements EmbeddingProvider {
  ...
  constructor(
    ...
    private readonly fallback: Float32Array = new Float32Array([1, 0, 0]),
  ) {}
  ...
  embedDocument(text: string): Promise<Float32Array> {
    return Promise.resolve(this.vectors.get(text) ?? this.fallback);
  }
...
async function setup(vectors: Map<string, Float32Array> = new Map()) {
  const state = StateService.open(
    join(mkdtempSync(join(tmpdir(), 'yc-memory-service-')), 'state.db'),
  );
...
describe('MemoryService.store', () => {
  it('stores a new memory, embeds it and adds it to the vector index', async () => {
    const { state, index, service } = await setup(
      new Map([['a useful pattern', new Float32Array([1, 0, 0])]]),
    );
  ...
  it('reports an exact duplicate on identical namespace+content without creating a second row', async () => {
    ...
    expect(
      memories.get(
        (first as { status: 'stored'; memory: { id: string } }).status === 'stored'
          ? (first as never as { memory: { id: string } }).memory.id
          : '',
      ),
    ).toBeTruthy();
    state.close();
  ...
  it('reports a near-duplicate (cosine >= 0.95) even when the text differs', async () => {
    ...
    const first = await service.store({
      namespace: 'patterns',
      content: 'original phrasing of the pattern',
      summary: null,
      sourceSwarmId: null,
      sourceTaskId: null,
      confidence: 0.5,
    ...
    const second = await service.store({
      namespace: 'patterns',
      content: 'a differently worded near-duplicate',
      summary: null,
      sourceSwarmId: null,
      sourceTaskId: null,
      confidence: 0.5,
  ...
  it('does not flag a near-duplicate in a DIFFERENT namespace as a duplicate, but still does within the same namespace', async () => {
    ...
    const first = await service.store({
      namespace: 'solutions',
      content: 'a solution about retries',
      summary: null,
      sourceSwarmId: null,
      sourceTaskId: null,
      confidence: 0.5,
    ...
    const second = await service.store({
      namespace: 'failures',
      content: 'a differently worded failure about retries',
      summary: null,
      sourceSwarmId: null,
      sourceTaskId: null,
      confidence: 0.5,
...
describe('MemoryService.feedback', () => {
  it('increases confidence on a helpful verdict', async () => {
    const { state, memories, service } = await setup(
      new Map([['content', new Float32Array([1, 0, 0])]]),
    );
