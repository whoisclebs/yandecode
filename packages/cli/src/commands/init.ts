...
registerCommand((program) => {
  program
    ...
    .action(async (opts: { force: boolean }) => {
      ...
      const counts = report.files.reduce<Record<string, number>>(
        (acc, f) => ({ ...acc, [f.action]: (acc[f.action] ?? 0) + 1 }),
        {},
      );
      process.stdout.write(`YandeCode initialized in ${report.root}\n`);
      process.stdout.write(
        `  managed files: ${Object.entries(counts)
          .map(([k, v]) => `${v} ${k}`)
          .join(', ')}\n`,
      );
      ...
      process.stdout.write(
        `  config: ${report.configCreated ? 'created yandecode.json' : 'kept yandecode.json'}\n`,
      );
