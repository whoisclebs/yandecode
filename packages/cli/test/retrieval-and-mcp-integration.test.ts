...
describe('rag_search end-to-end with the hash embedding test double', () => {
  it('indexes a small repository and returns a matching chunk over MCP', async () => {
    ...
    writeFileSync(join(dir, 'auth.ts'), 'export function login(user: string): boolean {\n  return checkPassword(user);\n}\n');
    ...
    const r = await client.callTool({ name: 'rag_search', arguments: { query: 'login password check', limit: 5 } });
