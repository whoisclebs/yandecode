...
const byName = (results: ReturnType<typeof runDoctor>, name: string) => results.find((r) => r.name === name)!;
...
describe('runDoctor', () => {
  ...
  it('flags old Node and missing Claude Code', () => {
    ...
    const results = runDoctor({ cwd, probeVersion: () => null, nodeVersion: 'v20.1.0', yandecodeVersion: '0.1.0' });
  ...
  it('passes every Part 1 check after init and warns on edited managed files', async () => {
    ...
    let results = runDoctor({ cwd, probeVersion: probeAllOk, nodeVersion: 'v22.22.3', yandecodeVersion: '0.1.0' });
    for (const name of ['Config', 'Managed files', 'Hooks', 'MCP', 'CLAUDE.md', 'SQLite', 'WAL', 'Schema', 'FTS5']) {
    ...
    results = runDoctor({ cwd, probeVersion: probeAllOk, nodeVersion: 'v22.22.3', yandecodeVersion: '0.1.0' });
